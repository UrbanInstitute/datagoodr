...
#' Determine whether a vector should be treated as a factor (internal)
#'
#' Examines a vector and applies multiple heuristics to decide if it
#' can be considered a factor. Handles character, numeric, integer,
#' logical, and date types, and optionally converts binary variables to factors.
#'
#' @param x A vector to test.
#' @param unique.p Numeric; proportion of unique values threshold (default 0.05).
#' @param unique.n Integer; maximum number of unique values threshold (default 100).
#' @param top.n Integer; number of top frequent levels to check (default 25).
#' @param max.variance Numeric; maximum variance for numeric columns to be considered factor-like (default 2).
#' @param b.to.f Logical; whether to convert binary vectors (0/1 or single value) to factor (default FALSE).
#'
#' @return Logical; `TRUE` if the vector meets criteria to be treated as a factor, `FALSE` otherwise.
#'
#' @keywords internal
#' @noRd
...
#' Determine which columns in a data frame should be treated as factors (internal)
#'
#' Applies `is_factor()` to each column of a data frame and returns a logical vector
#' indicating which columns meet the criteria to be considered factors.
#'
#' @param df A data frame.
#'
#' @return A named logical vector, where `TRUE` indicates the column should be treated as a factor.
#'
#' @keywords internal
#' @noRd
...
#' Recast eligible columns of a data frame as factors (internal)
#'
#' Uses `is_factor_df()` to identify columns in a data frame that meet
#' criteria to be treated as factors, and converts those columns to factors.
#'
#' @param df A data frame.
#'
#' @return The same data frame, with eligible columns converted to factors.
#'
#' @keywords internal
#' @noRd
