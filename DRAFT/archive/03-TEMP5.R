...
f_to_txt <- function( fkable, f.lab ) {
  cat( paste0( "##### ", f.lab, "\n\n" ) )
  cat( paste0( fkable, " \n" ), sep="" )
  cat( "\n\n" )
}
...
v_to_txt <- function( x, x.lab ) {

  if( class(x) == "character" ) { 
    to_txt_string( x, x.lab ) }

  if( class(x) == "data.frame" ) { 
    fkable <- knitr::kable(x) 
    f_to_txt( fkable, f.lab=x.lab ) }
}
