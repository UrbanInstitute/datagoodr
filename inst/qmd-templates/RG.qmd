...
format:
  html:
    ...
    toc-depth: 5
    toc-expand: true
    output-file: file_name_placeholder
    output-ext: html
  pdf: 
    output-file: file_name_placeholder
    output-ext: "pdf"
execute:
  keep-md: true
  output: file_name_placeholder
params: 
  dgf_file:  "../DGF/DGF.xlsx"
---
...
# cat(data_file)
filetype.dgf <- tools::file_ext(data_file)
# cat(filetype.dgf)

# dgf <- readxl::read_excel(data_file)
if(filetype.dgf == "csv"){
    dgf <- readr::read_csv(data_file)
  }else if(filetype.dgf %in% c("xls", "xlsx")){
    dgf <- readxl::read_excel(data_file)
  }else{
    mess <- "DGF file cannot be read. Abort mission."
    cat(mess)
    knitr::knit_exit()
  }
# 
...
#   print(head(dgf))
# } else {
#   cat("Data frame 'dgf' does not exist.")
# }

```


```{r split-data, echo=F, message=FALSE, include=FALSE}
...
## Needed for layouts
## This is not needed when using the functions in 03*.R outside of a package build. 
## But inside of a package build environment, these need to be global variables
